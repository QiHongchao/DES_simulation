...
if(overwrite)
  save.dta13(model.hb$eventhistory,("./women/output/women_current.dta"))



## CURRENT STRATEGY - VALIDATION 
## NOTE: validation model requires recall as early as 12 weeks, as observed
## NOTE: validation model therefore uses different attendance (time to return) model parameters
...
params=list("hb"=c(visit.coef=-0.5723, rettime.coef=0.0118, age1.coef=0.0674, age2.coef=-0.0546, age3.coef=0.1116, age4.coef=0.1888, age5.coef=0.1733, eth.coef=-0.4193, blood2.coef=0.0098, blood3.coef=0.0963, blood4.coef=0.0879, blood5.coef=0.0776, blood6.coef=0.0182, blood7.coef=0.0732, blood8.coef=0.2769, cons=13.3709), ## hb recovery
            ...
            "defer"=c(cons=-0.5287, hb1.coef=0, hb2.coef=0, hb3.coef=0),
            "othdefer"=c(cons=-2.8513), 
            "strategy"=c(current=1, optimal=0, atrisk=0, indemand=0, atrisk.thresh=0.9, indemand.thresh=0.7, lowhbdef.recall=12, vlowhbdef.recall=52, othdef.recall=4, prob.thresh=0.9, min.recall=16, curr.recall=12, length=52, thresh=12.5, hb.min=8.7, hb.max=20.8) # indicators for recall strategy type
)
...
model.hb<-des(N=10000, params=params, agedist=agedist, exagedist=exagedist, hbdist=hbdist, ethdist=ethdist, blooddist=blooddist, prevdefdist=prevdefdist, prevdondist=prevdondist, blupdist=blupdist, blupsedist=blupsedist, recall=recall.curr, attend=attend.dist,
              ...
              fixedse52dist=fixedse52dist)
if(overwrite)
  save.dta13(model.hb$eventhistory,("./women/output/women_current_validation.dta"))
